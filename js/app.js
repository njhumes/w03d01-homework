...
function makeGame(){
    $('.game-levels').append(`<p id="hunger-level">Hunger: ${bart.hunger}</p>`);
    $('.game-levels').append(`<p id="boredom-level">Boredom: ${bart.boredom}</p>`);
    $('.game-levels').append(`<p id="sleep-level">Sleepiness: ${bart.sleepiness}</p>`);
...
function increaseLevels(){
    if(game.time % 2 === 0){
        bart.hunger += 1;
        $('#hunger-level').text(`Hunger: ${bart.hunger}`);
    }
    if(game.time % 3 === 0){
        bart.boredom += 2;
        $('#boredom-level').text(`Boredom: ${bart.boredom}`);
    }
    if(game.time % 5 === 0){
        bart.sleepiness += 3;
        $('#sleep-level').text(`Sleepiness: ${bart.sleepiness}`);
    }
    if(game.time % 15 === 0){
        bart.age += 1;
